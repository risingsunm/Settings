...
public class WifiSettings extends SettingsPreferenceFragment
    ...
    // su2 cw - start //
    private final static String ACTION_CW_STATE_CHANGED = "su2.intent.action.cw.state.change";
    private final static String CW_STATE = "cwstate";
    private final static String CW_AP_NETWORK_ID = "cw_network_id";
    private static int mCWState = -1;
    private static int mNetworkID = -1;
    private static String mWifiDialogSSID;
    // su2 cw - end //
    ...
    public WifiSettings() {
        ...
        // su2 cw - start //
        mFilter.addAction(ACTION_CW_STATE_CHANGED);
        // su2 cw - end //
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        ...
        if (!mInXlSetupWizard) {
            ...
            menu.add(Menu.NONE, MENU_ID_SCAN, 0, R.string.wifi_menu_scan)
                    ...
                    .setShowAsAction(MenuItem.SHOW_AS_ACTION_NEVER); //SHOW_AS_ACTION_IF_ROOM mzikun del menu display
            menu.add(Menu.NONE, MENU_ID_ADD_NETWORK, 0, R.string.wifi_add_network)
                    .setIcon(R.drawable.ic_menu_add_network)
                    .setEnabled(wifiIsEnabled)
                    .setShowAsAction(MenuItem.SHOW_AS_ACTION_NEVER); //SHOW_AS_ACTION_IF_ROOM mzikun del menu display
    ...
    // su2 cw add begin
    public static boolean isBelongCWSSID(String ssid) {
    	if(ssid == null)
    		return false;
      
        if(ssid.equals("ChinaNet"))
            return true;
        else if(ssid.equals("ChinaNet_CW"))
            return true;
        else if(ssid.equals("ChinaNet_HomeCW"))
            return true;
        else
            return false;
    }
        ...
        if (mInXlSetupWizard) {
            ...
            if (preference instanceof AccessPoint) {
                ...
                if (mSelectedAccessPoint.networkId != INVALID_NETWORK_ID) {
                    //su2 cw add start
                    if(!isBelongCWSSID(mSelectedAccessPoint.ssid))
                    ...
                    if (isSupportFMC && isCTAPExist) {
                        for (String ctap : CT_AP) {
                            if (ctap.equals(mSelectedAccessPoint.ssid)) {
                               if(!isBelongCWSSID(mSelectedAccessPoint.ssid))
    ...
    private void updateAccessPoints() {
		...
		for( AccessPoint accessPoint : accessPoints) {
			if(isBelongCWSSID(accessPoint.ssid))
				accessPoint.updateCW(mNetworkID, mCWState);
		}
		// su2 cw end
    ...
    private void handleEvent(Context context, Intent intent) {
	 ...
	 // su2 cw - start //
        else if(ACTION_CW_STATE_CHANGED.equals(action)) {
            mCWState = intent.getIntExtra(CW_STATE, -1);
            mNetworkID = intent.getIntExtra(CW_AP_NETWORK_ID, -1);
//            Slog.i("wifi-setting", "updateCWAccessPoint() cwState=" + mCWState);
//            Slog.i("wifi-setting", "updateCWAccessPoint() network id = " + mNetworkID);
            updateCWAccessPoint(mNetworkID, mCWState);
        }
        // su2 cw - end //
    ...
    // su2 cw - start //
    private void updateCWAccessPoint(int networkid, int state) {
	...
	for (int i = getPreferenceScreen().getPreferenceCount() - 1; i >= 0; --i) {
            ...
            if (preference instanceof AccessPoint) {
                ...
                if(accessPoint.updateCW(networkid, state)) {
            		return;
            	}
            }
        }
    ...
    private void updateConnectionState(DetailedState state) {
        ...
        //zliang add for WIFI 2012.07.25
    ...
    /* su2 cw add begin */ 
    private void notifyCWDisconnect() {
	  	if(SystemProperties.get("ro.config.su2.cw", "false").equals("true")) {
	  		String actApssid = mWifiManager.getConnectionInfo().getSSID();
	  		if (actApssid != null &&
	  		   (actApssid.equals("ChinaNet")  ||
	  		   actApssid.equals("ChinaNet_CW")  ||
	  		   actApssid.equals("ChinaNet_HomeCW") )) {		
	  			Intent intent = new Intent("CW_UNREGISTER_ACTION");
    ...
    /* package */ void submit(WifiConfigController configController) {
        ...
        switch(networkSetup) {
		 ...
		 // su2 cw - start //
                 String dialogSSID = configController.getSSID();
                 if(configController.isEdit() && isBelongCWSSID(dialogSSID)) {
		    mWifiDialogSSID = dialogSSID;
                    this.showDialog_addNetworkFailed();
//	            Slog.i("cw", "wifi setting onClick()  fail");
                    return;
                }
                //su2 cw - end //
    ...
    // su2 cw - start //
	private void showDialog_addNetworkFailed() {
		//AlertDialog.Builder builder = new AlertDialog.Builder(this); 
		AlertDialog.Builder builder = new AlertDialog.Builder(getActivity()); 
		builder.setMessage(this.getText(R.string.cw_wifi_ssid_same)); 
		builder.setPositiveButton(this.getText(R.string.cw_add_network_failed_button), new DialogInterface.OnClickListener() { 
		           public void onClick(DialogInterface dialog, int id) {
		        	   showDialog(null, true);
		        	   if(mDialog != null)
		        		   //mDialog.setSSID(mWifiDialogSSID);
		        		   mDialog.getController().setSSID(mWifiDialogSSID);
		           } 
		       }) ;
