...
class Summary {
    ...
    // su2 cw - start //
    static String get(Context context, int state) {
        if(state < 0)
            return "";
        String[] formats = context.getResources().getStringArray(R.array.cw_status);
        if (state >= formats.length || formats[state].length() == 0) {
            return null;
        }
        return String.format(formats[state], null);
    }
    // su2 cw - end //
}
