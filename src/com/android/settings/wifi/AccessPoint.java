...
// WAPI++
    ...
    private static int getSecurity(ScanResult result) {
	 ...
	 if (result.capabilities.contains("WAPI-PSK")) {
              Log.w(TAG, "private getSecurity: WAPI-PSK Etay1" + result.capabilities);
           return SECURITY_WAPI_PSK;
 	 } else if (result.capabilities.contains("WEP")) {
             Log.w(TAG, "private getSecurity: WEP " + result.capabilities);
            return SECURITY_WEP;
        } else if (result.capabilities.contains("PSK")) {
             Log.w(TAG, "private getSecurity: PSK " + result.capabilities);
            return SECURITY_PSK;
...
//WAPI++
		case SECURITY_WAPI_PSK:
              Log.w(TAG, "getSecurityString: SECURITY_WAPI_PSK");
		...
		case SECURITY_WAPI_CERT:
              Log.w(TAG, "getSecurityString: SECURITY_WAPI_CERT");
...
//WAPI--
    ...
    private static PskType getPskType(ScanResult result) {
        ...
        boolean wapi_psk = result.capabilities.contains("WAPI-PSK"); //ETAY
        if (wpa2 && wpa) {
    ...
    public int compareTo(Preference preference) {
    	...
    	// su2 cw - start //
        boolean isCW = WifiSettings.isBelongCWSSID(ssid);
	boolean isOtherCW = WifiSettings.isBelongCWSSID(other.ssid);
	if(isCW != isOtherCW) {
		return (isCW)? -1 : 1;
	}
	// su2 cw - end //
    ...
    // su2 cw - start //
    private void refreshCWOnly() {
	if(mCwState < 0)
		return;
	setSummary(Summary.get(getContext(), mCwState));
    }
    private void refreshCW() {
        if(mCwState < 0) {
            refresh();
        }
        else {
    ...
    boolean updateCW(int networkid, int state) {
       
        if (this.networkId != - 1 && this.networkId == networkid) {
            mCwState = state;
            refreshCW();
            return true;
        }
